...
def getZigzag(tiker='BTC', per=10):
    ...
    for i in range(1, len(df.index) - 1):
        if df.iloc[i][3] > df.iloc[i-1][3] and df.iloc[i][3] > df.iloc[i+1][3]:
    ...
    zigzag = zigzag1
    if len(zigzag1.index) < len(zigzag2.index): zigzag = zigzag2

    zigzag.to_csv('csv/' + tiker + '_zigzag' + str(per) + '.csv')
